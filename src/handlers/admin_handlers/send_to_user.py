...
@router.message(
    VPNOrder.admin_menu, F.text == "Отправить файл конфигурации или сообщение"
)
async def cmd_send_telegram_id_conf(message: Message, state: FSMContext):
    await state.update_data(prev="admin_menu")
    await message.answer(
        "Пришлите telegram_id пользователя которому хотите отправить файл конфигурации или сообщение",
...
@router.message(
    VPNOrder.send_conf, F.text == "Отправить пользователю текстовое сообщение"
)
...
async def cmd_check_message(message: Message, state: FSMContext):
    ...
    if message.from_user:
        await message.answer(
            f"Нажмите ✅ ОТПРАВИТЬ, чтобы отправить пользователю с id {tg_id}\nСообщение: {sended_message}",
            reply_markup=admin_waiting_keyboard(),
        )
            ...
            try:
                ...
                if tg_id_int:
                    ...
                    if user:
                        ...
                        await message.answer(
                            text=f"Пользователь с i {tg_id_int} успешно найден! Нажмите продолжить чтобы перейти к загрузке файла",
                            reply_markup=admin_continue_keyboard(),
                        )
                    else:
                        await message.answer(
                            text=f"Пользователь с id {tg_id_int} не найден!"
                        )
                        return
            except:
                await message.answer(
                    text="telegram id пользователя должен быть числом!"
                )
                return
        else:
...
async def cmd_send_config_file(message: Message, state: FSMContext):
    await message.answer(
        "Отправьте файл конфигурации для пользователя", reply_markup=back_button()
    )
                ...
                if message.document.file_name.endswith(
                    ...
                    if message.document.file_size:
                        if message.document.file_size > 5 * 1024 * 1024:
                            ...
                            await message.answer(
                                "Файл конфигурации успешно принят! Нажмите продолжить чтобы перейти к отправке",
                                reply_markup=admin_continue_keyboard(),
                            )
                else:
...
async def cmd_wait_for_continue(message: Message, state: FSMContext):
    data = await state.get_data()
    await message.answer(
        f"Нажмите <b>'отправить'</b>, чтобы отправить файл конфигурации пользовтелю с telegram_id: {data["tg_id_to_send_config"]}",
        reply_markup=admin_waiting_keyboard(),
    )
    ...
    try:
        if message.bot:
            await message.bot.send_document(
                chat_id=tg_id,
                document=file_id,
                caption=f"🔑 <b>Ваш файл конфигурации</b>\n",
            )
