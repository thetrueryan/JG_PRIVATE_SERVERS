...
class AsyncCore:
    ...
    async def update_paid_status(
        ...
        async with async_session_factory() as session:
            ...
            if order:
                ...
                logger.warning(
                    f"Ордер по invoice_id {invoice_id} не удалось найти и обновить."
                )
    ...
    async def get_user_by_tg_id(telegram_id: int) -> int | None:
        async with async_session_factory() as session:
            query = select(UsersOrm.id).filter(UsersOrm.telegram_id == telegram_id)
            res = await session.execute(query)
    ...
    async def get_order_by_id(order_id: int):
        async with async_session_factory() as session:
            query = select(OrdersOrm).filter(OrdersOrm.id == order_id)
            res_raw = await session.execute(query)
                ...
                if order.status == "paid":
                    ...
                    if paid_at:
                        ...
                        if new_duration:
                            ...
                            order.expires_at = current_date + timedelta(
                                days=new_duration_days_sum
                            )
                        if new_price:
    ...
    async def get_orders_list():
        async with async_session_factory() as session:
            stmt = select(OrdersOrm).where(
                OrdersOrm.status == "paid" and OrdersOrm.expires_at != None
            )
