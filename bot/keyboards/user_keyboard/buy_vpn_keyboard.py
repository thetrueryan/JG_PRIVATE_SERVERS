...
def select_country_menu():
    ...
    builder.row(KeyboardButton(text="🇺🇸 США"), KeyboardButton(text="🇮🇳 Индия"))
...
def select_traffic_menu():
    builder = ReplyKeyboardBuilder()
    builder.row(KeyboardButton(text="⚡ 50 mb/s"), KeyboardButton(text="🚀 1.0 gb/s"))
    builder.row(KeyboardButton(text="↩️ Назад"))
    return builder.as_markup(resize_keyboard=True)
...
def select_payment_menu():
    builder = ReplyKeyboardBuilder()
    builder.row(KeyboardButton(text="💎 Cryptobot"), KeyboardButton(text="💵 Fiat"))
    builder.row(KeyboardButton(text="↩️ Назад"))
    return builder.as_markup(resize_keyboard=True)
        ...
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="Проверить оплату", callback_data=f"check_payment:{invoice_id}"
                ),
            ],
        ]
    )
