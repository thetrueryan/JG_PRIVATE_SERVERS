        ...
        if message.text:
            try:
                ...
                if len(params_list) >= 4:
                    ...
                    await message.answer(f"Данные для ввода в таблицу:\n{params}\n\n Нажмите ✅ ДОБАВИТЬ Чтобы добавить сервер в таблицу", reply_markup=admin_add_server_keyboard())
                ...
                await message.answer(f"Не удалось получить данные о сервере по ошибке: <b>{e}</b>")
                return
...
async def cmd_add_server(message: Message, state: FSMContext):
    try:
        ...
        if params:
            ...
            await AsyncCore.add_server(country, vpn_type, traffic, price_per_day, user_id)
            await message.answer("Сервер успешно добавлен в таблицу!", reply_markup=back_button())        
    except Exception as e:
        await message.answer(f"Не удалось добавить сервер в таблицу по ошибке: <b>{e}</b>")